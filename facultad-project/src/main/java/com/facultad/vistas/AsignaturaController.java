...
public class AsignaturaController {
    ...
    @FXML
    private ComboBox<Instituto> cmbInstituto;
    @FXML
    private ComboBox<Docente> cmbDocenteResponsable;

    // ========== LISTA DE CARRERAS ==========
    @FXML
    private ListView<Carrera> listCarreras;
    @FXML
    private Button btnQuitarCarrera;
    ...
    @FXML
    private TableColumn<Asignatura, String> colInstituto;
    @FXML
    private TableColumn<Asignatura, String> colResponsable;
    @FXML
    private TableColumn<Asignatura, Integer> colCarreras;
    ...
    private void configurarTabla() {
        ...
        colInstituto.setCellValueFactory(
        ...
        colResponsable.setCellValueFactory(cellData -> {
            ...
            return new SimpleStringProperty(
                    responsable != null ? responsable.getApellidos() + ", " + responsable.getNombres() : "No asignado");
        });

        colCarreras.setCellValueFactory(cellData -> new javafx.beans.property.SimpleIntegerProperty(
                cellData.getValue().getCarreras().size()).asObject());
    ...
    private void cargarCarrerasAsignadas() {
        if (asignaturaSeleccionada != null) {
            // 🔥 AHORA ES DIRECTO: getCarreras() devuelve List<Carrera>
            List<Carrera> carreras = asignaturaSeleccionada.getCarreras();
    ...
    private void editarAsignatura() {
        ...
        try {
            ...
            if (docenteNuevo != null) {
                // Verificar si el NUEVO docente ya tiene un cargo para este instituto
                boolean yaTieneCargo = docenteNuevo.getCargos().stream()
                        .anyMatch(c -> c.getInstituto().equals(asignaturaSeleccionada.getInstituto()));

                if (!yaTieneCargo) {
                    // No tiene cargo, crear uno nuevo
                    CargoDocente cargo = new CargoDocente();
                    cargo.setDocente(docenteNuevo);
                    cargo.setInstituto(asignaturaSeleccionada.getInstituto());
                    cargo.setDedicacionHoras(10); // Valor fijo

                    cargoDocenteService.agregarCargo(cargo);
                    System.out.println("✅ Nuevo cargo creado para " + docenteNuevo.getNombreCompleto());
                }
            }

            // Si había un docente anterior y AHORA NO HAY docente, no hacemos nada especial
            // (el cargo del docente anterior sigue existiendo - podría tener otras
            // asignaturas)
    ...
    private void quitarCarrera() {
        ...
        if (carrera == null || asignaturaSeleccionada == null)
            return;
